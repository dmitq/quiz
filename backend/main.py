...
async def create_quiz(quiz: schemas.QuizCreate, user: schemas.User = Depends(services.get_current_user), db: orm.Session = Depends(services.get_db)):
    return await services.create_quiz(user, db, quiz)


@app.get("/api/quizes", response_model=List[schemas.Quiz])
async def get_user_quizes(user: schemas.User = Depends(services.get_current_user), db: orm.Session = Depends(services.get_db)):
...
async def get_quiz_edit(quiz_id: int, db: orm.Session = Depends(services.get_db), user: schemas.User = Depends(services.get_current_user)):
...
async def get_quiz_bio(quiz_id: int, db: orm.Session = Depends(services.get_db)):
    return await services.get_quiz_bio(quiz_id, db)

@app.get("/api/quizes/{quiz_id}/leaderboard", status_code=200)
async def get_leaderboard(quiz_id: int, db: orm.Session = Depends(services.get_db)):
    return await services.get_leaderboard(quiz_id, db)


@app.post("/api/check_answers", status_code=200)
async def check_answers(data: schemas.AnswersCheck, db: orm.Session = Depends(services.get_db)):
